...
- 支持自定义模型价格：**平均价模式**（一个价格对所有 token 统一计价，匹配总量计费）或分项模式（输入/输出/缓存读），支持任意货币（内置 CNY 汇率，其余可自定义）；设置页可添加/编辑/删除，无价格模型一键跳转设置
- 支持**显示货币**：设置页可切换（USD/CNY/EUR/JPY 等）并编辑汇率，面板费用卡片、趋势图、分布图与托盘均按显示货币渲染，可点击费用卡片在美元/显示货币间切换
- 一键**导入社区价格**：从本项目的 GitHub Release 拉取社区维护的中文区常用模型价格（GLM/Qwen/DeepSeek/Kimi/MiniMax/Doubao 等，CNY 平均价），合并到本地且不覆盖已设置的价格
